...
- `.yamibaito/prompts` は互換目的でシンボリックリンクに統一し、コピー運用を廃止する。
...
- `.yamibaito/prompts` のシンボリックリンク作成は必須とし、`ln -s` 失敗時はエラーメッセージを出して初期化を停止する（symlink 非対応環境はサポート対象外）。
...
- 依存関係: シンボリックリンクの扱いが環境依存（特に一部ファイルシステム）であり、symlink 非対応環境はサポート対象外となる。
